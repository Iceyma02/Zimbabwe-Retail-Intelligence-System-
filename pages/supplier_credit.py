...
def compute_priority_score(row):
    status_score = {"STOPPED": 1.0, "LIMITED_CREDIT": 0.6, "ACTIVE": 0.1}.get(row.get("supplier_status", "ACTIVE"), 0.2)
    overdue_score = min(row.get("overdue_days", 0) / 60, 1.0)
    outstanding_pct = row.get("outstanding_usd", 0) / max(row.get("amount_usd", 1), 1)
...
def layout():
    try:
        ...
        confidence_block = html.Div([
            ...
            ], style={"display": "flex", "alignItems": "center", "justifyContent": "space-between", "gap": "40px", "flexWrap": "wrap"})
        ...
        total_overdue = df[df["overdue_days"] > 0]["outstanding_usd"].sum() if "overdue_days" in df.columns else 0
        ...
        # Status chart - FIXED: Use separate layout update
        ...
        # FIX: Create a copy and update separately
