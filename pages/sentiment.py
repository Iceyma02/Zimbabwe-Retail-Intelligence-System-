"""Customer Sentiment & Feedback — Page 13"""
import dash
...
# Generate simulated sentiment data in-memory
def get_sentiment_data():
    stores = get_stores()
    np.random.seed(77)
    store_nps = {}
    for _, s in stores.iterrows():
        # Bigger stores tend to have slightly lower NPS (busier, longer queues)
        base_nps = np.random.randint(28, 72)
        store_nps[s["store_id"]] = {"store_name": s["name"], "city": s["city"], "nps": base_nps}
    ...
    for store_id, info in store_nps.items():
        for month in months:
            nps_variation = info["nps"] + np.random.randint(-8, 8)
    ...
    return pd.DataFrame(records), store_nps
...
def layout():
    return html.Div([
        ...
        html.Div([
            html.Div(id="sent-kpis", style={"display": "flex", "gap": "14px", "marginBottom": "20px"}),
            html.Div([
                html.Div([dcc.Graph(id="sent-nps-chart", config={"displayModeBar": False}, style={"height": "300px"})],
                         style={"flex": "1.2", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="sent-complaints-pie", config={"displayModeBar": False}, style={"height": "300px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
            ], style={"display": "flex", "gap": "14px", "marginBottom": "14px"}),
            html.Div([
                html.Div([dcc.Graph(id="sent-trend", config={"displayModeBar": False}, style={"height": "260px"})],
                         style={"flex": "1.5", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="sent-heatmap", config={"displayModeBar": False}, style={"height": "260px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
            ], style={"display": "flex", "gap": "14px"}),
            dcc.Interval(id="sent-load", interval=99999999, n_intervals=0, max_intervals=1)
        ], style={"padding": "20px 28px"})
    ])


@callback(
    Output("sent-kpis", "children"),
    Output("sent-nps-chart", "figure"),
    Output("sent-complaints-pie", "figure"),
    Output("sent-trend", "figure"),
    Output("sent-heatmap", "figure"),
    Input("sent-load", "n_intervals")
)
def update_sentiment(_):
    df, store_nps = get_sentiment_data()
    ...
    fig_nps = go.Figure(go.Bar(
        ...
        text=nps_by_store["nps_score"].apply(lambda x: f"{x:.0f}"),
        textposition="outside"
    ))
    fig_nps.add_vline(x=60, line_dash="dash", line_color="#22c55e40", annotation_text="Good (60)")
    fig_nps.add_vline(x=40, line_dash="dash", line_color="#eab30840", annotation_text="Fair (40)")
    fig_nps.update_layout(**CHART_LAYOUT,
                           title={"text": "NPS Score by Store", "font": {"color": "#ccc", "size": 13}},
                           xaxis_range=[0, 110])

    # Complaints pie
    comp_totals = df.groupby("complaint_category")["complaint_count"].sum().sort_values(ascending=False).reset_index()
    fig_pie = go.Figure(go.Pie(
        labels=comp_totals["complaint_category"],
        values=comp_totals["complaint_count"],
        hole=0.4,
    ...
    fig_trend.update_layout(**CHART_LAYOUT,
                             title={"text": "National NPS Trend Over Time", "font": {"color": "#ccc", "size": 13}},
                             yaxis_range=[0, 100])

    # Complaint heatmap: store × category
    ...
    fig_hm = go.Figure(go.Heatmap(
        ...
        colorscale=[[0, "#0d0d0d"], [0.5, "#00c853"], [1, "#ff8a9b"]],
        showscale=True
    ))
    fig_hm.update_layout(**CHART_LAYOUT,
                          title={"text": "Store × Complaint Category Heatmap", "font": {"color": "#ccc", "size": 13}})
    fig_hm.update_xaxes(tickangle=-30)

    return [html.Div(k, style={"flex": 1}) for k in kpis], fig_nps, fig_pie, fig_trend, fig_hm
