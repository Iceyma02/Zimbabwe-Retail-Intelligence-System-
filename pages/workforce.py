"""Workforce Intelligence — Page 14 with Retailer Filter"""
import dash
...
def layout():
    return html.Div([
        ...
        html.Div(id="workforce-content", style={"padding": "20px 28px"})
    ])


@callback(
    Output("workforce-content", "children"),
    Input("active-retailer", "data")
)
def update_workforce(retailer):
    try:
        staff = get_staff(retailer)
        revenue = get_store_revenue_summary(30, retailer)
        
        if staff.empty:
            return html.Div([
                html.Div("No workforce data available", 
        ...
        fig_lr.update_layout(**CHART_LAYOUT, barmode="group",
                              title={"text": f"Revenue vs Labour Cost by Store - {retailer_name}", "font": {"color": "#ccc", "size": 13}})
        ...
        fig_hc.update_layout(**CHART_LAYOUT,
                              title={"text": f"Staff Headcount by Store - {retailer_name}", "font": {"color": "#ccc", "size": 13}})
        ...
        fig_ot.update_layout(**CHART_LAYOUT,
                              title={"text": f"Overtime Hours This Month - {retailer_name}", "font": {"color": "#ccc", "size": 13}})
        ...
        fig_tenure.update_layout(**CHART_LAYOUT,
                                  title={"text": f"Avg Staff Tenure by Store (years) - {retailer_name}", "font": {"color": "#ccc", "size": 13}})
        ...
        table = html.Div([
            html.Div(f"Store Labour Summary - {retailer_name}", style={"color": "#888", "fontSize": "11px",
