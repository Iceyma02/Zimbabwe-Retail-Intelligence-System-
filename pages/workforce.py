...
def layout():
    return html.Div([
        page_header("Workforce Intelligence", "Staff headcount, labour costs vs revenue, overtime and vacancies", "fa-users"),
        html.Div([
            html.Div(id="wf-kpis", style={"display": "flex", "gap": "14px", "marginBottom": "20px"}),
            html.Div([
                html.Div([dcc.Graph(id="wf-labour-rev", config={"displayModeBar": False}, style={"height": "300px"})],
                         style={"flex": "1.5", "background": "#161616", "border": "1px solid #222", "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="wf-headcount", config={"displayModeBar": False}, style={"height": "300px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222", "borderRadius": "10px", "padding": "16px"}),
            ], style={"display": "flex", "gap": "14px", "marginBottom": "14px"}),
            html.Div([
                html.Div([dcc.Graph(id="wf-overtime", config={"displayModeBar": False}, style={"height": "260px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222", "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="wf-tenure", config={"displayModeBar": False}, style={"height": "260px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222", "borderRadius": "10px", "padding": "16px"}),
                html.Div(id="wf-summary-table",
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222", "borderRadius": "10px", "padding": "20px"}),
            ], style={"display": "flex", "gap": "14px"}),
            dcc.Interval(id="wf-load", interval=99999999, n_intervals=0, max_intervals=1)
        ], style={"padding": "20px 28px"})
    ])


@callback(
    Output("wf-kpis", "children"),
    Output("wf-labour-rev", "figure"),
    Output("wf-headcount", "figure"),
    Output("wf-overtime", "figure"),
    Output("wf-tenure", "figure"),
    Output("wf-summary-table", "children"),
    Input("wf-load", "n_intervals")
)
def update_workforce(_):
    staff = get_staff()
    revenue = get_store_revenue_summary(30)
    df = staff.merge(revenue[["store_id", "total_revenue", "total_profit"]], on="store_id", how="left")
    df["labour_pct_revenue"] = (df["monthly_labour_cost_usd"] / df["total_revenue"] * 100).round(1)
    df["revenue_per_head"] = (df["total_revenue"] / df["headcount"]).round(0)

    total_headcount = staff["headcount"].sum()
    total_labour = staff["monthly_labour_cost_usd"].sum()
    total_vacancies = staff["vacancies"].sum()
    avg_labour_pct = df["labour_pct_revenue"].mean()

    kpis = [
        kpi_card("Total Staff", str(int(total_headcount)), None, None, "fa-users", "#3b82f6"),
        ...
        kpi_card("Open Vacancies", str(int(total_vacancies)), None, None, "fa-user-plus", "#eab308"),
    ]

    # Labour vs Revenue grouped bar
    fig_lr = go.Figure()
    ...
    fig_lr.add_trace(go.Bar(
        x=df["store_name"], y=df["monthly_labour_cost_usd"],
    ...
    fig_hc.update_layout(**CHART_LAYOUT, title={"text": "Staff Headcount by Store", "font": {"color": "#ccc", "size": 13}})

    # Overtime
    df_ot = df.sort_values("overtime_hours_this_month", ascending=False)
    ot_colors = ["#ef4444" if h > 60 else "#f97316" if h > 30 else "#22c55e" for h in df_ot["overtime_hours_this_month"]]
    fig_ot = go.Figure(go.Bar(
        x=df_ot["store_name"], y=df_ot["overtime_hours_this_month"],
        marker_color=ot_colors,
        text=df_ot["overtime_hours_this_month"].apply(lambda x: f"{x}h"),
        textposition="outside"
    ))
    fig_ot.add_hline(y=40, line_dash="dash", line_color="#666", annotation_text="40h alert line")
    fig_ot.update_layout(**CHART_LAYOUT, title={"text": "Overtime Hours This Month", "font": {"color": "#ccc", "size": 13}})
    fig_ot.update_xaxes(tickangle=-30)

    # Tenure
    fig_tenure = go.Figure(go.Bar(
        x=df["store_name"], y=df["avg_tenure_years"],
        marker_color=["#22c55e" if t > 4 else "#eab308" if t > 2 else "#ef4444" for t in df["avg_tenure_years"]],
        text=df["avg_tenure_years"].apply(lambda x: f"{x:.1f}y"),
        textposition="outside"
    ))
    fig_tenure.update_layout(**CHART_LAYOUT, title={"text": "Avg Staff Tenure by Store (years)", "font": {"color": "#ccc", "size": 13}})
    fig_tenure.update_xaxes(tickangle=-30)

    # Summary table
    headers = ["Store", "Staff", "Labour $", "Labour %", "Rev/Head", "Vacancies"]
    header_row = html.Tr([
        html.Th(h, style={"color": "#666", "fontSize": "11px", "padding": "7px 8px",
    ...
    summary_title = html.Div("Store Labour Summary", style={
    ...
    return [html.Div(k, style={"flex": 1}) for k in kpis], fig_lr, fig_hc, fig_ot, fig_tenure, table
