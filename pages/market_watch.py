...
dash.register_page(__name__, path="/market-watch", name="Zimbabwe Market Watch", order=15)
...
def layout():
    return html.Div([
        page_header("Zimbabwe Market Watch", "Exchange rates, inflation, fuel, load shedding — local factors affecting retail", "fa-globe-africa"),
        html.Div([
            # Live indicators strip
            html.Div(id="mw-live-strip", style={"marginBottom": "20px"}),

            html.Div([
                html.Div([dcc.Graph(id="mw-zig-chart", config={"displayModeBar": False}, style={"height": "280px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="mw-inflation-chart", config={"displayModeBar": False}, style={"height": "280px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
                html.Div([dcc.Graph(id="mw-fuel-chart", config={"displayModeBar": False}, style={"height": "280px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
            ], style={"display": "flex", "gap": "14px", "marginBottom": "14px"}),

            html.Div([
                html.Div([dcc.Graph(id="mw-loadshed-chart", config={"displayModeBar": False}, style={"height": "240px"})],
                         style={"flex": "1", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "16px"}),
                html.Div(id="mw-seasonal-calendar",
                         style={"flex": "1.4", "background": "#161616", "border": "1px solid #222",
                                "borderRadius": "10px", "padding": "20px"}),
            ], style={"display": "flex", "gap": "14px"}),

            dcc.Interval(id="mw-load", interval=99999999, n_intervals=0, max_intervals=1)
        ], style={"padding": "20px 28px"})
    ])


@callback(
    Output("mw-live-strip", "children"),
    Output("mw-zig-chart", "figure"),
    Output("mw-inflation-chart", "figure"),
    Output("mw-fuel-chart", "figure"),
    Output("mw-loadshed-chart", "figure"),
    Output("mw-seasonal-calendar", "children"),
    Input("mw-load", "n_intervals")
)
def update_market(_):
    ...
    strip = html.Div([
        html.Div([
            ...
            html.Div([
                html.Div(ind["label"], style={"color": "#666", "fontSize": "10px", "textTransform": "uppercase", "letterSpacing": "0.8px"}),
    ...
    # ZiG rate chart
    fig_zig = go.Figure(go.Scatter(
        x=df["date"], y=df["usd_zig_rate"],
        line={"color": "#eab308", "width": 2},
        fill="tozeroy", fillcolor="rgba(234,179,8,0.07)"
    ))
    fig_zig.update_layout(**CHART_LAYOUT,
                           title={"text": "USD / ZiG Exchange Rate (180 Days)", "font": {"color": "#ccc", "size": 13}})

    # Inflation chart
    fig_inf = go.Figure(go.Scatter(
        x=df["date"], y=df["inflation_rate_percent"],
        line={"color": "#ef4444", "width": 2},
        fill="tozeroy", fillcolor="rgba(239,68,68,0.07)"
    ))
    fig_inf.update_layout(**CHART_LAYOUT,
                           title={"text": "Inflation Rate % (180 Days)", "font": {"color": "#ccc", "size": 13}})

    # Fuel price chart
    fig_fuel = go.Figure(go.Scatter(
        x=df["date"], y=df["fuel_price_usd_per_litre"],
        line={"color": "#f97316", "width": 2},
        fill="tozeroy", fillcolor="rgba(249,115,22,0.07)"
    ))
    fig_fuel.update_layout(**CHART_LAYOUT,
                            title={"text": "Fuel Price USD/Litre (180 Days)", "font": {"color": "#ccc", "size": 13}})
    ...
    fig_ls = go.Figure(go.Bar(
        x=weekly_ls["date"], y=weekly_ls["load_shedding_hours"],
        marker_color=ls_colors
    ))
    fig_ls.add_hline(y=8, line_dash="dash", line_color="#ef444450", annotation_text="Critical (8h)")
    fig_ls.update_layout(**CHART_LAYOUT,
                          title={"text": "Weekly Avg Load Shedding Hours", "font": {"color": "#ccc", "size": 13}})
    ...
    return strip, fig_zig, fig_inf, fig_fuel, fig_ls, html.Div(calendar_items)
