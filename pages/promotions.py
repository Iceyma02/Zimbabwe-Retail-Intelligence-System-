...
def layout():
    return html.Div([
        page_header("Promotions ROI Tracker","Did our promotions actually work? Revenue lift vs margin impact","fa-tags"),
        html.Div([
            html.Div(id="promo-kpis", style={"display":"flex","gap":"14px","marginBottom":"20px"}),
            html.Div([
                html.Div([dcc.Graph(id="promo-roi-chart",config={"displayModeBar":False},style={"height":"280px"})],
                         style={"flex":1,"background":"#161616","border":"1px solid #222","borderRadius":"10px","padding":"16px"}),
                html.Div([dcc.Graph(id="promo-lift-chart",config={"displayModeBar":False},style={"height":"280px"})],
                         style={"flex":1,"background":"#161616","border":"1px solid #222","borderRadius":"10px","padding":"16px"}),
            ...
            html.Div([
                ...
                html.Div(id="promo-table")
            ], style={"background":"#161616","border":"1px solid #222","borderRadius":"10px","padding":"20px"}),
            dcc.Interval(id="promo-load",interval=99999999,n_intervals=0,max_intervals=1)
        ], style={"padding":"20px 28px"})
    ])

@callback(
    Output("promo-kpis","children"),Output("promo-roi-chart","figure"),
    Output("promo-lift-chart","figure"),Output("promo-table","children"),
    Input("promo-load","n_intervals")
)
def update_promos(_):
    df = get_promotions()
    ...
    kpis = [
        ...
        kpi_card("Avg ROI",f"{avg_roi:.1f}%",None,None,"fa-percent","#22c55e" if avg_roi>0 else "#ef4444"),
    ...
    fig_roi.update_layout(**CHART_LAYOUT,title={"text":"ROI % by Promotion","font":{"color":"#ccc","size":13}})
    fig_roi.update_xaxes(tickangle=-30)

    df["revenue_lift"] = df["promo_revenue"] - df["pre_promo_revenue"]
    fig_lift = go.Figure()
    fig_lift.add_trace(go.Bar(x=df["promo_name"].str[:18],y=df["pre_promo_revenue"],name="Pre-Promo",marker_color="#444"))
    fig_lift.add_trace(go.Bar(x=df["promo_name"].str[:18],y=df["promo_revenue"],name="During Promo",marker_color="#00c853",opacity=0.8))
    fig_lift.update_layout(**CHART_LAYOUT,barmode="overlay",title={"text":"Revenue Lift by Promotion","font":{"color":"#ccc","size":13}})
    ...
    return [html.Div(k,style={"flex":1}) for k in kpis],fig_roi,fig_lift,table
