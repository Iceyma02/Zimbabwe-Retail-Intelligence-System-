...
def layout():
    return html.Div([
        ...
        html.Div([
            html.Div(id="reorder-kpis", style={"display":"flex","gap":"14px","marginBottom":"20px"}),
            html.Div([
                dcc.Graph(id="reorder-urgency-chart", config={"displayModeBar":False}, style={"height":"280px"})
            ], style={"background":"#161616","border":"1px solid #222","borderRadius":"10px","padding":"16px","marginBottom":"14px"}),
            html.Div([
                html.Div("Reorder Queue — Prioritised by Urgency", style={"color":"#888","fontSize":"11px",
            ...
            dcc.Interval(id="reorder-load", interval=99999999, n_intervals=0, max_intervals=1)
        ], style={"padding":"20px 28px"})
    ])

@callback(
    Output("reorder-kpis","children"),
    Output("reorder-urgency-chart","figure"),
    Output("reorder-table","children"),
    Input("reorder-load","n_intervals")
)
def update_reorder(_):
    ...
    needs_reorder = needs_reorder.sort_values("urgency_score", ascending=False)
    ...
    fig.update_layout(**CHART_LAYOUT, title={"text":"Top 20 Items by Reorder Urgency Score","font":{"color":"#ccc","size":13}},
    ...
    for _, row in needs_reorder.head(50).iterrows():
        ...
        rows.append(html.Tr([
            ...
            html.Td(html.Div(style={"width":f"{min(row['urgency_score']/needs_reorder['urgency_score'].max()*100,100):.0f}%",
    ...
    return [html.Div(k, style={"flex":1}) for k in kpis], fig, table
