"""Reorder Optimizer — Page 8 with Retailer Filter & Supplier Risk"""
import dash
...
def get_reorder_data(retailer):
    try:
        ...
        for col in ["product_id", "product_name", "store_name", "supplier_name"]:
            if col in inv.columns:
                if isinstance(inv[col], pd.DataFrame):
                    ...
                    inv[col] = inv[col].apply(flatten_string)
        ...
        sales_90 = get_sales(90, retailer)
        product_forecast = {}
...
def layout():
    return html.Div([
        ...
        html.Div(id="reorder-content", style={"padding": "20px 28px"})
    ])


@callback(
    Output("reorder-content", "children"),
    Input("active-retailer", "data")
)
def update_reorder(retailer):
    try:
        needs_reorder = get_reorder_data(retailer)
                ...
                html.Div([
                    html.Div([
                        html.Div([
                            ...
                            html.Div(f"No items currently need reordering for {retailer_name}. All stock levels are above reorder points.",
        ...
        chart_layout.update({
            "title": {"text": f"Top 20 Items by Reorder Urgency Score ({retailer_name})", "font": {"color": "#ccc", "size": 13}},
        ...
        return html.Div([
            ...
            html.Div([dcc.Graph(figure=fig, config={"displayModeBar": False}, style={"height": "500px"})],
                     style={"background": "#161616", "border": "1px solid #222", "borderRadius": "10px",
                            "padding": "16px", "marginBottom": "14px"}),
            html.Div([
                html.Div(f"Reorder Queue — Prioritised by Urgency ({retailer_name})", style={
        ...
        return html.Div([
            page_header("Reorder Optimizer", "Smart reorder suggestions", "fa-rotate"),
            html.Div([
                html.Div([
