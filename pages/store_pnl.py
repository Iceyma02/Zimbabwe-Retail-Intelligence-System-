"""Store P&L Engine — Page 4 with Retailer Filter"""
import dash
...
def layout():
    ...
    return html.Div([
        ...
        html.Div(id="pnl-content", style={"padding": "20px 28px"}),
        dcc.Store(id="pnl-store-data")
    ])
...
def update_pnl_ui(retailer):
    stores = get_stores(retailer)
    if stores.empty:
        return html.Div(f"No stores available for {retailer}", 
    ...
    return html.Div([
        ...
        dcc.Store(id="pnl-retailer", data=retailer)
    ])
...
@callback(
    ...
    Input("pnl-retailer", "data")
)
def update_pnl(store_id, months, retailer):
    try:
        costs_df = get_store_costs(retailer)
        sales_df = get_sales(months * 30, retailer)

        if sales_df.empty or costs_df.empty:
            empty_fig = go.Figure()
            empty_fig.update_layout(**CHART_LAYOUT, title={"text": "No data available for this selection"})
